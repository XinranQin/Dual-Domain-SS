...
parser.add_argument('--test_epoch', type=int, default=400, help='epoch number of test')
parser.add_argument('--layer_num', type=int, default=9, help='phase number')
parser.add_argument('--learning_rate', type=float, default=1e-4, help='learning rate')
parser.add_argument('--group_num', type=int, default=1, help='group number for training')
parser.add_argument('--cs_ratio', type=int, default=10, help='from {10, 25, 40}')
...
model_dir = "./%s/DDSS_layer_%d_ratio_%d_lr_%.4f_unsuper" % (
...
if test_epoch > 0:
    ...
    model.load_state_dict(torch.load('%s/net_params_%d.pkl' % (pre_model_dir, test_epoch)))
...
output_data = "CS ratio is %d, Avg PSNR/SSIM for %s is %.2f/%.4f, Epoch number of model is %d \n" % (
    cs_ratio, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), test_epoch)
print(output_data)
