...
class Network(torch.nn.Module):
    ...
    def forward(self, Phix, Phi, Qinit, output_minusnoise,xinit=None):
...
model_dir = "./%s/DDSS_%d_ratio_%d_lr_%.4f_unsuper" % (
    args.model_dir, layer_num, cs_ratio, learning_rate)
...
for epoch_i in range(start_epoch, end_epoch + 1):
    for data in rand_loader:
        ...
        noise = (torch.FloatTensor(Phix.size()).normal_(mean=0, std=2 / 255).cuda())
        [x_output, loss_layers_sym] = model(Phix + noise, Phi, Qinit, Phix - noise)

        x1 = x_output
        PhiNx = torch.mm(x1, torch.transpose(Phi, 0, 1))
        x_input1 = PhiNx
        [x_output1, loss_layers_sym] = model(x_input1, Phi, Qinit, PhiNx)

        re = (x_output-x_output1).view(-1,1,33,33)
        blinmask = torch.ones_like(re)
        blinmask = F.dropout(blinmask, 0.5) * 0.5
        blinmask = torch.where(blinmask == 0,-1 * blinmask, blinmask)
        noise1 = (blinmask*re.detach()).view(-1,1089)

        PhiNx = torch.mm(x1.detach() + noise1, torch.transpose(Phi, 0, 1))
        x_input1 = PhiNx
        [x_output1, loss_layers_sym] = model(x_input1, Phi, Qinit,
                                             PhiNx)
        loss_measurement = torch.mean(torch.pow(x_output1-(x_output-noise1), 2))
        loss_image = torch.mean(
            torch.pow(torch.mm(x_output, torch.transpose(Phi, 0, 1)) - (Phix-noise), 2))
        loss_all = loss_measurement   + 0.1 * (loss_image)
        optimizer.zero_grad()
        loss_all.backward()
        optimizer.step()
    if epoch_i % 10 == 0:
        ...
        torch.save(model.state_dict(), "./%s/net_withnonoise_params_%d.pkl" % (model_dir, epoch_i))  # save only the parameters
        ...
        with torch.no_grad():
            for img_no in range(ImgNum):
                ...
                Phix = torch.mm(batch_x, torch.transpose(Phi, 0, 1))
                Phix = torch.mm(batch_x, torch.transpose(Phi, 0, 1))
                X_output = torch.mm(Phix, torch.transpose(Qinit, 0, 1))
                end = time()
